# Welcome 👋

Hi! I'm **[Mickaël Rakotoarinivo](https://www.linkedin.com/in/mickael-rakotoarinivo/)** – a **Data Engineer** specializing in **Apache Spark & Databricks**.

---

## 🚀 About Me

I thrive on designing **robust, scalable data pipelines** that optimize performance and ensure data quality. With **5 years** of hands-on experience, I'm currently mastering **Databricks** to advance my expertise in enterprise-level data engineering.

Currently seeking new challenges in **Switzerland** focused on Databricks and modern data stack technologies.

**📜 Certifications:**
- Databricks Certified Data Engineer (Associate Developer for Apache Spark)
- Databricks Certified Data Analyst
- [View my credentials](https://credentials.databricks.com/profile/faniriantsoatianamickalrakotoarinivo886659/wallet)

**📄 CV / Resume:**
- [English Version](pages/CV_Data_Engineer_EN.md)
- [French Version](pages/CV_Data_Engineer_FR.md)

---

## 🛠️ Tech Stack

**Core Data Engineering**

| Databricks | Apache Spark | Snowflake | dbt |
|:---:|:---:|:---:|:---:|
| <img src="https://github.com/DOX69/Databricks_for_data_engineer/blob/9bbd65be9604ac8f9da57d836831f5b63a8d8179/images/DBX.png?raw=true" width="50" alt="Databricks"/> | <img src="https://github.com/devicons/devicon/raw/master/icons/apachespark/apachespark-original-wordmark.svg" width="50" alt="Apache Spark"/> | <img src="https://github.com/DOX69/DOX69/blob/main/assets/SNOWFLAKE.png?raw=true" width="50" alt="Snowflake"/> | <img src="https://github.com/DOX69/DOX69/blob/main/assets/dbt-removebg-preview.png?raw=true" width="50" alt="dbt"/> |

**Languages & Tools**

| Python | SQL | Git | Azure | docker |
|:---:|:---:|:---:|:---:|:---:|
| <img src="https://github.com/DOX69/DOX69/blob/main/assets/python.png?raw=true" width="50" alt="Python"/> | <img src="https://github.com/DOX69/Databricks_for_data_engineer/blob/62cb87995265002231d4dda14092e2e76c7e4163/images/SQL.svg?raw=true" width="50" alt="SQL"/> | <img src="https://github.com/devicons/devicon/raw/master/icons/git/git-original-wordmark.svg" width="50" alt="Git"/> | <img src="https://github.com/devicons/devicon/raw/master/icons/azure/azure-original.svg" width="50" alt="Azure"/> | <img src="https://github.com/devicons/devicon/blob/master/icons/docker/docker-original.svg" width="50" alt="Docker"/> |

**Expertise Areas**
- ✅ ETL/ELT Pipeline Development & Automation
- ✅ Batch & Streaming Data Processing
- ✅ Data Ingestion & Autoloader
- ✅ CI/CD Integration & DevOps
- ✅ Data Quality & Governance
- ✅ Performance Optimization

---

## 💼 Professional Background

**Data Engineer @ Inpost Group** (Oct 2022 – Present)
- Architected scalable PySpark/SQL pipelines using Databricks Asset Bundles (DABs)
- Implemented Delta Lake for batch and streaming ETL processes
- Built CI/CD automation and DevOps workflows
- Technical lead for cross-functional data teams

**Data Analyst @ Hoist Finance** (May 2021 – Aug 2022)
- Automated workflows reducing manual processing by 40%
- Developed ETL pipelines for operational reporting
- Created interactive Tableau dashboards for stakeholder insights

**Education**
- 🎓 Master in Data Sciences – University of Lille (2020–2022)
- 📚 Bachelor in Economics & Management (2017–2020)

---
<p align="center">
 <img width="1000" src="assets/github-snake.svg" alt="snake"/>
</p>

## 📚 Featured Repositories

- [**Finance analysis**](https://github.com/DOX69/Bitcoin-analysis) - Creation of web app analysis for Bitcoin
- [**Databricks for Data Engineer**](https://github.com/DOX69/Databricks_for_data_engineer) – Comprehensive guide to Databricks concepts and implementations
- [**PySpark & Delta Lake Tutorials**](https://github.com/DOX69?tab=repositories) – Practical examples and best practices

---

## 🎯 What I'm Working On

Currently deepening expertise in:
- Advanced data optimization techniques
- Data governance & metadata management
- Real-time streaming architectures
- Production-ready testing frameworks

---

## 🏋️ Beyond Data

**Sports Enthusiast:** Fitness advocate – weight training, running, and Aikido keep my mind and body sharp 💪

**Finance Geek:** Passionate about financial education, investment strategies, and understanding monetary systems ₿📈

**Fun Fact:** Started my career in restaurants 🍳, pivoted to finance, and discovered data magic. Now blending culinary passion with data engineering – creating delicious insights! 🍽️📊

---

## 📞 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mickael-rakotoarinivo/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mickael.rakotoa@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/DOX69)

---

**📍 Location:** Crissier, Vaud, Switzerland  
**🔍 Status:** Open to opportunities in Data Engineering | Databricks Focus

*Last updated: January 2026*
